...
class Parameters:
    ...
    def __init__(self, node: Node):
        ...
        node.declare_parameter(name='connection_type', value=UART.CONNECTORTYPE_UART)
        ...
        try:
            ...
            if self.connection_type.value == UART.CONNECTORTYPE_UART:
...
#     def __init__(self,node:Node):
#         self.node = node
#         self.node.get_logger().info('Initializing parameters')

#         self.ros_topic_prefix = self.node.declare_parameter(name='ros_topic_prefix', value='bno055/')
#         self.connection_type = self.node.declare_parameter(name='connection_type', value='uart')
#         self.uart_port  = self.node.declare_parameter('uart_port', value='/dev/ttyUSB0')
#         self.uart_baudrate = self.node.declare_parameter('uart_baudrate', value=115200)
#         self.uart_timeout = self.node.declare_parameter('uart_timeout', value=0.1)

#         # sensor operation mode
#         self.operation_mode = self.node.declare_parameter('operation_mode', value=0x0C)
#         # placement_axis_remap defines the position and orientation of the sensor mount
#         self.placement_axis_remap = self.node.declare_parameter('placement_axis_remap', value='P1')
#          # scaling factor for acceleration
#         self.acc_factor = self.node.declare_parameter('acc_factor', value=100.0)
#         # scaling factor for magnetometer
#         self.mag_factor = self.node.declare_parameter('mag_factor', value=16000000.0)
#         # scaling factor for gyroscope
#         self.gyr_factor = self.node.declare_parameter('gyr_factor', value=900.0)
#         # determines whether to use default offsets or not
#         self.set_offsets = self.node.declare_parameter('set_offsets', value=False)

#         # +/- 2000 units (at max 2G) (1 unit = 1 mg = 1 LSB = 0.01 m/s2)
#         self.offset_acc = self.node.declare_parameter('offset_acc', value=registers.DEFAULT_OFFSET_ACC)
#         # +/- 6400 units (1 unit = 1/16 uT)
#         self.offset_mag = self.node.declare_parameter('offset_mag', value=registers.DEFAULT_OFFSET_MAG)
#         # +/- 2000 units up to 32000 (dps range dependent)               (1 unit = 1/16 dps)
#         self.offset_gyr = self.node.declare_parameter('offset_gyr', value=registers.DEFAULT_OFFSET_GYR)

#         # +/-1000 units
#         self.radius_acc = self.node.declare_parameter('radius_acc', value=registers.DEFAULT_RADIUS_ACC)
#         #  +/-960 units
#         self.radius_mag = self.node.declare_parameter('radius_mag', value=registers.DEFAULT_RADIUS_MAG)

#         # Sensor standard deviation squared (^2) defaults [x, y, z]
#         self.variance_acc = self.node.declare_parameter('variance_acc', value=registers.DEFAULT_VARIANCE_ACC)
#         self.variance_angular_vel = self.node.declare_parameter('variance_angular_vel', value=registers.DEFAULT_VARIANCE_ANGULAR_VEL)
#         self.variance_orientation = self.node.declare_parameter('variance_orientation', value=registers.DEFAULT_VARIANCE_ORIENTATION)
#         self.variance_mag = self.node.declare_parameter('variance_mag', value=registers.DEFAULT_VARIANCE_MAG)

#          # get the parameters - requires CLI arguments '--ros-args --params-file <parameter file>'
#         self.node.get_logger().info('Parameters set to:')
