...
class PurePursuit(Node):
    def __init__(self):
        ...
        print("csv_file_path: ", waypoint_path)
        ...
        self.tf_buffer_odom = Buffer()
        self.tf_listener_odom = TransformListener(self.tf_buffer_odom, self)    


        self.subscription = self.create_subscription(PoseWithCovarianceStamped,'/amcl_pose',self.amcl_pose_callback,10)

        self.drive_pub_ = self.create_publisher(Twist,self.drive_topic, 10)
    ...
    def amcl_pose_callback(self, msg):
        ...
        self.publish_drive_command(steering_angle=steering_angle, steering_limit=1.0, K_p=2.0, max_speed=5.5)
    ...
    def get_velocity(self, steering_angles: float, high_speed_factor: float, medium_speed_factor: float, slow_speed_factor: float, car_speed: float) -> float:
    ...
    def publish_drive_command(self, steering_angle, steering_limit, K_p, max_speed):
        ...
        if steering_angle < 0.0:
            ...
            drive_msgObj.angular.z = max(steering_angle, -self.deg_to_rad(steering_limit)) 

        else:
            # drive_msgObj.drive.steering_angle = min(steering_angle, self.deg_to_rad(steering_limit))  # Steering limit (positive)
            drive_msgObj.angular.z = min(steering_angle, self.deg_to_rad(steering_limit))  # Steering limit (positive)

        # Calculate the current velocity based on the steering angle
        # if self.brake  == False:
