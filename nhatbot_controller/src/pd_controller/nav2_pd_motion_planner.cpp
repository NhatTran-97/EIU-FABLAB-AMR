...
#include "nhatbot_controller/pd_nav2_motion_planner.hpp"
...
bool PDMotionPlanner::transformPlan(const std::string & frame)
{
  ...
  {
    RCLCPP_ERROR_STREAM(logger_, "Couldn't transform plan from frame " <<global_plan_.header.frame_id << " to frame " << frame);
    return false;
  }
...
PLUGINLIB_EXPORT_CLASS(nhatbot_controller::PDMotionPlanner, nav2_core::Controller)