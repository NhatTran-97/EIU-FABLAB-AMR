...
#include "nhatbot_controller/pure_pursuit.hpp"
...
void PurePursuit::controlLoop()
{
  ...
  RCLCPP_INFO(get_logger(), "curvature: %.2f", curvature); 
...
double PurePursuit::deg_to_rad(double deg) 
{
...
int main(int argc, char *argv[])
{
    ...
    auto node = std::make_shared<nhatbot_controller::PurePursuit>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}